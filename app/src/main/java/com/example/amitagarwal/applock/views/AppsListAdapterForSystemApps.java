...
public class AppsListAdapterForSystemApps extends BaseAdapter {

	ArrayList<SystemAppDataForDao> initialListViewData = new ArrayList<SystemAppDataForDao>();
	ArrayList<SystemAppDataForDao> partialData;
	...
	public AppsListAdapterForSystemApps(Context context, ArrayList<SystemAppDataForDao> listViewData,OnClickListener listener) {
		...
		sortApps();
		this.partialData = new ArrayList<SystemAppDataForDao>(initialListViewData);
		inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
	}

	public AppsListAdapterForSystemApps(Context context,boolean isRefreshing){
		this.isRefreshing = isRefreshing;
		inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
	}

	private void sortApps() {
		Collections.sort(initialListViewData, new Comparator<SystemAppDataForDao>() {

			@Override
			public int compare(SystemAppDataForDao lhs, SystemAppDataForDao rhs) {
	...
	private void initAppIconMap() {
		for(SystemAppDataForDao packageData:initialListViewData){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		viewHolder.isSystemApp.setText("System App");
	...
	public void removePackageAndRefreshViews(String pid) {
		...
		for(SystemAppDataForDao appDataForDao:initialListViewData){			
			if(appDataForDao.getPid().equalsIgnoreCase(pid)){
				initialListViewData.remove(appDataForDao);
				break;
			}
		}
		for(SystemAppDataForDao appDataForDao:partialData){			
			if(appDataForDao.getPid().equalsIgnoreCase(pid)){
